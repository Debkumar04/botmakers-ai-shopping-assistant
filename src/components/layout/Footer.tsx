...
export default function Footer() {
  return (
    <footer className="bg-[#09090F] border-t border-[#23232d] mt-6">
      {/* Sponsors */}
      <div className="mt-6">
        <div className="page-container mt-20">
          <h2 className="font-[Orbitron] text-center text-4xl font-bold uppercase tracking-wide">
            <span className="text-[#ef4444]">Sponsors</span>
          </h2>

          <div className="mt-10 flex flex-wrap justify-center items-center gap-8 md:gap-14">
            {SPONSORS.map((sponsor) => (
              ...
              >
                ...
                <span className="font-[Rajdhani] text-2xl md:text-3xl font-semibold text-[#8f8f98] uppercase tracking-wide">
                  {sponsor.logo}
                </span>
              </div>
            ))}
      ...
      <div className="page-container py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 md:gap-8">
          ...
          <div className="lg:col-span-5">
            ...
            <p className="max-w-[320px] text-[#7b7b88] text-base md:text-lg leading-7 md:leading-9">
          ...
          {/* Center — Quick Links */}
          <div className="lg:col-span-3 flex flex-col items-start md:items-center">
            <h3 className="font-[Orbitron] text-white uppercase text-lg md:text-xl mb-5 md:mb-8">
              Quick Links
            </h3>
            ...
            <ul className="space-y-3 md:space-y-5">
              {FOOTER_LINKS.quickLinks.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="text-[#8b8b95] text-lg md:text-xl font-[Rajdhani] hover:text-[#ef4444] transition"
                  >
                    {item.label}
                  </a>
                </li>
          ...
          {/* Right — Social Media */}
          <div className="lg:col-span-4 flex flex-col items-start md:items-end">
            <h3 className="font-[Orbitron] text-white uppercase text-lg md:text-xl mb-5 md:mb-8">
              Social Media
            </h3>
            ...
            <div className="flex gap-4 md:gap-5">
              {FOOTER_LINKS.socialMedia.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="w-10 h-10 rounded-xl border border-[#2d2d36] bg-[#12121a] flex items-center justify-center text-[#8b8b95] text-xl hover:border-[#ef4444] hover:text-[#ef4444] transition-all duration-300"
                >
